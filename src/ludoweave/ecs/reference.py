...
class ReferenceWorld:
        ...
        try:
            ...
            for entity_id in matched:
                ...
                for component_type, schema, capture_signature in columns:
                    stored = self._values[component_type][entity_id]
                    if capture_signature:
                        value, signature = _reference_copy_component_with_signature(
                            stored,
                            component_type,
                            schema,
    ...
    def _commit_query_row(self, spec: QuerySpec, row: QueryRowState) -> None:
        ...
        for component_type in spec.writable:
            ...
            schema = self._registry.schema_for_type(component_type)
            canonical, signature = _reference_copy_component_with_signature(
                row.values[index],
                component_type,
                schema,
...
def _reference_copy_component(
    ...
    *,
    ...
    copied, _signature = _reference_prepare_component_copy(
        value,
        component_type,
        schema,
        operation=operation,
...
def _reference_copy_component_with_signature(
    value: object,
    component_type: type[object],
    schema: ComponentSchema,
    *,
    ...
    copied, signature = _reference_prepare_component_copy(
        value,
        component_type,
        schema,
        operation=operation,
    ...
    if signature is None:
        raise RuntimeError("reference component signature capture invariant failed")
    return copied, signature


def _reference_prepare_component_copy(
    value: object,
    component_type: type[object],
    schema: ComponentSchema,
    *,
    ...
    try:
        ...
        signature = (
            tuple(_reference_signature_value(field_value) for _name, field_value in captured)
            if capture_signature
            else None
        )
        return copied, signature
