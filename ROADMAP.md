...
| In progress | M84 conventional archive entry-count preflight | Require both final end-of-central-directory entry counts to match the standard reader's parsed member count after M83 disk policy and before decoded-name policy, metadata, inventory, staging, or reads without a ZIP64 end-record parser, sentinel resolution, multi-volume assembler, workflow, producer, runtime, dependency, or release-authority change |
