...
| In progress | M90 local-header signature preflight | Require the four bytes at every parser-exposed local-header offset to match the local-file-header signature after M89 bounds policy and before names, metadata, inventory, staging, or reads with a narrow signature classifier but no local-header field parser, record-extent or inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
