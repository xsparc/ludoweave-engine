...
| Active | M97 local-header extraction-version consistency preflight | Require the bounded two-byte local extraction-version pair to match public central values after M96 and before decoded-name policy, metadata, inventory, staging, or reads without a supported-version allowlist, time/CRC/size comparison, inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
