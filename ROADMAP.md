...
| In progress | M89 local-header-offset bounds preflight | Require every parser-exposed local-header offset to remain strictly before the conventional central directory after M88 order policy and before names, metadata, inventory, staging, or reads without a local-header parser, record-extent or inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
