...
| Active | M94 local-header flag-consistency preflight | Require each two-byte local general-purpose flag field to equal the corresponding parser-exposed central `ZipInfo.flag_bits` after M93 and before decoded-name policy, metadata, inventory, staging, or reads without local compression-method or extra-field comparison, field-wide consistency checking, next-header or payload bounds, an inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
