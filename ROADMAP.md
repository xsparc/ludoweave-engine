...
| Active | M82 split-volume sample-member preflight | Reject parser-exposed nonzero `ZipInfo.volume` after established flag/extra-field/comment policy and before decoded-name policy, metadata, inventory, staging, or reads with a stable content-silent error and no raw end-record parser, multi-volume assembler, workflow, producer, runtime, dependency, or release-authority change |
