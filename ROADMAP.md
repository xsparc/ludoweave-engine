...
| Active | M77 NUL-suffixed sample-member name preflight | Exact `ZipInfo.orig_filename` NUL rejection before metadata, inventory, staging, or reads without a general normalized-name comparison, raw parser, workflow, producer, runtime, dependency, or release-authority change |
