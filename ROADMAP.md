...
| Active | M96 local-header extra-field consistency preflight | Require each bounded local extra field to exactly equal public central `ZipInfo.extra` after M95 and before decoded-name policy, metadata, inventory, staging, or reads without an extra-field semantics parser, broad extra-field ban, version/time/CRC/size or field-wide consistency checking, next-header or payload bounds, an inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
