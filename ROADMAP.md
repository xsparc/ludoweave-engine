...
| In progress | M91 fixed local-header-prefix bounds preflight | Require every parser-exposed offset to leave room for ZIP's 30-byte fixed local-header prefix before the conventional central directory after M90 signatures and before names, metadata, inventory, staging, or reads, using one prefix-bound classifier without a local-header field parser, record-extent or inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
