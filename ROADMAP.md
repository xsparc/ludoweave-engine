...
| Active | M95 local-header compression-method consistency preflight | Require each two-byte local compression method to equal the corresponding parser-exposed central `ZipInfo.compress_type` after M94 and before decoded-name policy, metadata, inventory, staging, or reads without local extra-field comparison, version/time/CRC/size or field-wide consistency checking, next-header or payload bounds, an inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
