...
| In progress | M93 local-header name-consistency preflight | Read each bounded local file-name after M92 and require exact raw-byte equality with the parser-exposed central name reconstructed under its central CP437/UTF-8 encoding before decoded-name policy, metadata, inventory, staging, or reads, without local-flag or extra-field comparison, next-header or payload bounds, an inter-member layout validator, workflow, producer, runtime, dependency, or release-authority change |
