...
| Active | M81 ZIP comment preflight | Reject parser-exposed non-empty archive and member comments after established flag/extra-field policy and before decoded-name policy, metadata, inventory, staging, or reads with stable content-silent errors and no raw ZIP parser, general comment scanner, workflow, producer, runtime, dependency, or release-authority change |
