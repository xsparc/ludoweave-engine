...
M0 and M1 are complete. The user has authorized milestone work through M6, but each milestone must still be implemented and fully validated on its own task-scoped branch before automatic commit, push, and pull-request creation. M2 commands, transactions, receipts/diffs, snapshots, replay/branching, and data-only workflow CLI adapters are complete and independently accepted locally; do not begin M3 until the M2 publication candidate passes hosted CI. WebGPU, MCP, physics, audio, networking, editor work, and native code remain out of scope until their assigned milestones. The complete-M1 benchmark records a substantial local 10,000-entity tick target miss; it does not by itself authorize native acceleration.
