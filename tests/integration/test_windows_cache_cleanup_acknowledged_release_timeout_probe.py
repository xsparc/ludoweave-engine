"""Test-only acknowledged-release Windows wait-timeout probe for M161."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M161 probes a wait timeout after acknowledged blocker release intent",
)

_CHILD = Path(__file__).parents[1] / "fixtures/windows_share_delete_acknowledged_release_child.py"
...
def _parse_phase(line: bytes) -> str:
    if not line.endswith(b"\n") or len(line) > _MAX_LINE_BYTES:
        raise RuntimeError("acknowledged-release blocker returned invalid output") from None
    try:
    ...
    if type(phase) is not str or phase not in {"ready", "release-held", "closed"}:
        raise RuntimeError("acknowledged-release blocker returned invalid output") from None
    return phase
...
def _read_phase(process: subprocess.Popen[bytes]) -> str:
    ...
    if stdout is None:
        raise RuntimeError("acknowledged-release blocker stdout is unavailable") from None
    ...
    try:
        ...
        raise RuntimeError("acknowledged-release blocker phase timed out") from None
    reader.join(timeout=_TIMEOUT_SECONDS)
    if reader.is_alive():
        raise RuntimeError("acknowledged-release blocker phase did not settle") from None
    if isinstance(result, BaseException):
        raise RuntimeError("acknowledged-release blocker phase failed") from result
    return _parse_phase(result)


def test_acknowledged_release_retains_blocker_until_explicit_close(tmp_path: Path) -> None:
    ...
    live_path.mkdir()
    candidate_path.write_bytes(b"m161-acknowledged-release-timeout")
    ...
    with filesystem_probe:
        ...
        if filesystem_name.casefold() != "ntfs":
            pytest.skip("M161 acknowledged-release fixture requires an NTFS pytest volume")
    ...
    try:
        ...
        assert candidate_path.read_bytes() == b"m161-acknowledged-release-timeout"
        ...
        assert displaced_candidate.read_bytes() == b"m161-acknowledged-release-timeout"
    finally:
