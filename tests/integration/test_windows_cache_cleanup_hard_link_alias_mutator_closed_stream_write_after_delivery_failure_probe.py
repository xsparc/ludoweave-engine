"""Test-only Windows closed-stream write after buffered delivery failure probe."""
...
from test_windows_cache_cleanup_hard_link_alias_mutator_closed_stream_flush_after_delivery_failure_probe import (
    _send_invalid_sequence_then_assert_closed_stream_flush,  # pyright: ignore[reportPrivateUsage]
)
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M198 probes write after buffered close delivery failure",
)
...
def _send_invalid_sequence_then_assert_closed_stream_write(
    ...
    return_code = _send_invalid_sequence_then_assert_closed_stream_flush(process)
    assert stdin.closed
...
def test_write_raises_after_closed_flush_following_buffered_delivery_failure(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            mutator_return_code = _send_invalid_sequence_then_assert_closed_stream_write(mutator)
            ...
            with pytest.raises(OSError) as blocked_after_closed_stream_write:
                coordination_path.rename(displaced_path)
            assert blocked_after_closed_stream_write.value.winerror == _ERROR_SHARING_VIOLATION
