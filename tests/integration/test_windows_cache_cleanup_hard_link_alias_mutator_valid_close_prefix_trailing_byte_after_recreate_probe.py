"""Test-only Windows alias-mutator trailing byte after valid close prefix probe."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M191 probes one trailing byte after a valid post-recreation close prefix",
)

_VALID_CLOSE_PREFIX_WITH_TRAILING_INVALID_BYTE = b"!?"
_EXPECTED_RETURN_CODE = 0
_ERROR_SHARING_VIOLATION = 32


def _send_valid_close_prefix_with_trailing_byte_and_assert_settlement(
    ...
    assert stdin.write(_VALID_CLOSE_PREFIX_WITH_TRAILING_INVALID_BYTE) == len(
        _VALID_CLOSE_PREFIX_WITH_TRAILING_INVALID_BYTE
    )
    stdin.flush()
    assert _read_alias_mutator_event(process) == "closed"
    assert not stdin.closed
    stdin.close()
    assert stdin.closed
    return_code = process.wait(timeout=_TIMEOUT_SECONDS)
    assert return_code == _EXPECTED_RETURN_CODE
    assert stdout.read(_MAX_LINE_BYTES + 1) == b""
    assert stderr.read(_MAX_LINE_BYTES + 1) == b""
    return return_code


def test_valid_close_prefix_ignores_one_trailing_byte_after_recreate(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            assert mutator.poll() == _EXPECTED_RETURN_CODE
            assert guardian.poll() is None
            ...
            with pytest.raises(OSError) as blocked_after_trailing_byte:
                coordination_path.rename(displaced_path)
            assert blocked_after_trailing_byte.value.winerror == _ERROR_SHARING_VIOLATION
    ...
    assert mutator_return_code == _EXPECTED_RETURN_CODE
    assert mutator.returncode == _EXPECTED_RETURN_CODE
