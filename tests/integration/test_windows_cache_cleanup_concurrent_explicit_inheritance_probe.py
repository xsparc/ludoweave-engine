"""Test-only simultaneous Windows explicit handle-list isolation probe."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M167 probes simultaneous explicit Windows handle-list isolation",
)
...
def _denied_rename(root: Path) -> _NativeRenameResult:
    return _attempt_native_child_rename(root)


@pytest.mark.parametrize("release_order", ((_LABELS[0], _LABELS[1]), (_LABELS[1], _LABELS[0])))
def test_simultaneous_explicit_lists_isolate_distinct_blockers(
    tmp_path: Path,
    ...
    payloads = {label: f"m167-explicit-isolation-{label}".encode() for label in _LABELS}
    ...
    with filesystem_probe:
        ...
        if filesystem_name.casefold() != "ntfs":
            pytest.skip("M167 explicit-list fixture requires an NTFS pytest volume")
    ...
    with blocker_probe:
        ...
        def coordinate_inheritability(handle: int, inheritable: bool) -> None:
            ...
            with coordination_lock:
                ...
                if restoring_handles == set(handles.values()):
                    both_restoring.set()
        ...
        def launch_explicit_child(
            ...
            *,
            ...
            with coordination_lock:
                ...
                if created_handles == set(handles.values()):
                    both_created.set()
        ...
        try:
            ...
            assert all(original_get_handle_inheritable(handle) for handle in handles.values())

            assert both_created.wait(timeout=_TIMEOUT_SECONDS)
            assert created_handles == set(handles.values())
            assert all(thread.is_alive() for thread in threads.values())
            assert all(original_get_handle_inheritable(handle) for handle in handles.values())

            permit_launch_return.set()
            assert both_restoring.wait(timeout=_TIMEOUT_SECONDS)
            assert restoring_handles == set(handles.values())
            assert all(thread.is_alive() for thread in threads.values())
            assert all(original_get_handle_inheritable(handle) for handle in handles.values())
            permit_restore.set()
            ...
            for label in _LABELS:
                assert _denied_rename(roots[label]) == denied
            ...
            assert _attempt_native_child_rename(roots[first]) == _NativeRenameResult(
                succeeded=True,
                error_code=0,
            )
            assert _denied_rename(roots[second]) == denied
            assert not os.path.lexists(live_paths[first])
            assert displaced_paths[first].is_dir()
            assert live_paths[second].is_dir()
            assert not os.path.lexists(displaced_paths[second])

            _release_inherited_blocker(processes[second])
            assert _attempt_native_child_rename(roots[second]) == _NativeRenameResult(
                succeeded=True,
                error_code=0,
            )
            for label in _LABELS:
