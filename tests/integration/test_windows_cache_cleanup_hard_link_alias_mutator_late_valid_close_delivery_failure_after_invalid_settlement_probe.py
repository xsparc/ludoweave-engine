"""Test-only Windows late valid-close delivery-failure probe."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M194 probes late valid-close delivery after invalid settlement",
)

_INVALID_PREFIX_WITH_VALID_CLOSE_SUFFIX = b"?!"
_LATE_VALID_CLOSE_TOKEN = b"!"
_INVALID_CONTROL_EXIT_CODE = 5
_ERROR_SHARING_VIOLATION = 32


def _send_invalid_sequence_then_assert_late_valid_close_delivery_failure(
    ...
    assert stdin.write(_LATE_VALID_CLOSE_TOKEN) == len(_LATE_VALID_CLOSE_TOKEN)
    assert not stdin.closed
...
def test_late_valid_close_is_buffered_then_delivery_fails_after_invalid_settlement(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            mutator_return_code = (
                _send_invalid_sequence_then_assert_late_valid_close_delivery_failure(mutator)
            )
            ...
            with pytest.raises(OSError) as blocked_after_late_close:
                coordination_path.rename(displaced_path)
            assert blocked_after_late_close.value.winerror == _ERROR_SHARING_VIOLATION
