"""Test-only Windows repeated close after buffered delivery failure probe."""
...
from test_windows_cache_cleanup_hard_link_alias_mutator_buffered_close_delivery_failure_after_invalid_settlement_probe import (
    _send_invalid_sequence_then_assert_buffered_close_delivery_failure,  # pyright: ignore[reportPrivateUsage]
)
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M196 probes repeated close after buffered close delivery failure",
)
...
def _send_invalid_sequence_then_assert_repeated_buffered_close(
    ...
    return_code = _send_invalid_sequence_then_assert_buffered_close_delivery_failure(process)
    assert stdin.closed
...
def test_repeated_close_is_no_op_after_buffered_close_delivery_failure(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            mutator_return_code = _send_invalid_sequence_then_assert_repeated_buffered_close(
                mutator
            )
            ...
            with pytest.raises(OSError) as blocked_after_repeated_close:
                coordination_path.rename(displaced_path)
            assert blocked_after_repeated_close.value.winerror == _ERROR_SHARING_VIOLATION
