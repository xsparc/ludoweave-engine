"""Test-only Windows alias-mutator control EOF after recreation probe."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M189 probes control-pipe EOF after Windows hard-link alias recreation",
)

_CONTROL_EOF_EXIT_CODE = 5
_ERROR_SHARING_VIOLATION = 32


def _close_alias_mutator_control_and_assert_eof(
...
def test_control_pipe_eof_after_recreate_leaves_alias_present(tmp_path: Path) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            assert mutator.poll() == _CONTROL_EOF_EXIT_CODE
            assert guardian.poll() is None
            ...
            with pytest.raises(OSError) as blocked_after_control_eof:
                coordination_path.rename(displaced_path)
            assert blocked_after_control_eof.value.winerror == _ERROR_SHARING_VIOLATION
    ...
    assert mutator_return_code == _CONTROL_EOF_EXIT_CODE
    assert mutator.returncode == _CONTROL_EOF_EXIT_CODE
