"""Test-only Windows buffered-close delivery-failure probe."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M195 probes buffered close delivery after invalid settlement",
)
...
def _send_invalid_sequence_then_assert_buffered_close_delivery_failure(
...
def test_late_valid_close_is_buffered_then_close_delivery_fails_after_invalid_settlement(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            mutator_return_code = (
                _send_invalid_sequence_then_assert_buffered_close_delivery_failure(mutator)
            )
