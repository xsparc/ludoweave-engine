"""Test-only Windows blocker invalid-control-token probe for M158."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M158 probes an invalid Windows blocker control token",
)

_INVALID_CONTROL_TOKEN = b"?"
_INVALID_CONTROL_EXIT_CODE = 4


def test_invalid_control_token_closes_blocker_before_rename_retry(
    tmp_path: Path,
) -> None:
    ...
    live_path.mkdir()
    candidate_path.write_bytes(b"m158-invalid-control-token")
    ...
    with filesystem_probe:
        ...
        if filesystem_name.casefold() != "ntfs":
            pytest.skip("M158 invalid-token fixture requires an NTFS pytest volume")
    ...
    try:
        ...
        assert candidate_path.read_bytes() == b"m158-invalid-control-token"
        ...
        assert stdin.write(_INVALID_CONTROL_TOKEN) == len(_INVALID_CONTROL_TOKEN)
        stdin.flush()
        stdin.close()
        assert stdin.closed
        ...
        assert displaced_candidate.read_bytes() == b"m158-invalid-control-token"
    finally:
        _close_child(blocker)

    assert blocker.returncode == _INVALID_CONTROL_EXIT_CODE
    assert displaced_candidate.read_bytes() == b"m158-invalid-control-token"
