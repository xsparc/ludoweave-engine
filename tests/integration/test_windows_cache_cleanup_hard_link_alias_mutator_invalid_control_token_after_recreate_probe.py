"""Test-only Windows alias-mutator invalid token after recreation probe."""
...
pytestmark = pytest.mark.skipif(
    sys.platform != "win32",
    reason="M190 probes an invalid control token after Windows alias recreation",
)
...
def _send_invalid_alias_mutator_control_and_assert_settlement(
...
def test_invalid_control_token_after_recreate_leaves_alias_present(tmp_path: Path) -> None:
    ...
    try:
        ...
        with identity_probe, lock_probe:
            ...
            mutator_return_code = _send_invalid_alias_mutator_control_and_assert_settlement(mutator)
            ...
            with pytest.raises(OSError) as blocked_after_invalid_control:
                coordination_path.rename(displaced_path)
            assert blocked_after_invalid_control.value.winerror == _ERROR_SHARING_VIOLATION
