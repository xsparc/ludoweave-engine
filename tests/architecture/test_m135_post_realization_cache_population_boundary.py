"""Protect explicit, complete-success-before-write M135 cache population."""
...
_PROTECTED = {
    ...
    "scripts/smoke_asset_realization_wheel.py": "0ef1a7719ef10fdc8258b9aaea9169abd3f40e76d5cb22447a458cdc15ffc15c",
    "docs/rfcs/0117-add-read-only-cache-assisted-asset-realization.md": "0e7e03a5a208c8c3736f13bc5593123faeb0ee685b7304126dc97266274db195",
}
...
def test_m135_retains_ci_dependencies_and_prior_asset_contracts() -> None:
    assert {path: _sha256(_ROOT / path) for path in _PROTECTED} == _PROTECTED


def test_population_realizes_completely_before_write_authority_and_publication() -> None:
    source = (_ROOT / "src/ludoweave/assets/population.py").read_text(encoding="utf-8")
    block = source.split("def populate_asset_build_cache(", 1)[1].split("\ndef ", 1)[0]
    read = block.index("writable=False")
    realize = block.index("realize_asset_build_plan")
    write = block.index("writable=True")
    publish = block.index(".publish(")
    assert read < realize < write < publish
    for forbidden in (
        "requests",
        "urllib",
        "socket",
...
def test_cli_verifies_and_acquires_before_population_composition() -> None:
    cli = (_ROOT / "src/ludoweave/tools/cli.py").read_text(encoding="utf-8")
    assert 'source_command == "asset-cache-populate"' in cli
    block = cli.split("def _run_asset_cache_populate(", 1)[1].split("\ndef ", 1)[0]
    ...
    assert block.index("_acquire_asset_build_inputs") < block.index("populate_asset_build_cache")
    assert "AssetCacheStore" not in block
    assert ".publish(" not in block


def test_m135_has_behavior_installed_and_documentation_evidence() -> None:
    for path in (
        "tests/unit/test_asset_cache_population.py",
        "tests/integration/test_asset_cache_cli.py",
        "scripts/smoke_asset_cache_population_wheel.py",
        "docs/rfcs/0118-add-post-realization-cache-population.md",
    ):
    ...
    assert "m135" in combined
    assert "ludoweave.asset-cache-population/1" in combined
    assert "complete realization" in combined
    assert "per-entry" in combined
