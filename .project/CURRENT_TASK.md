...
- **Task:** Bind the complete bounded encoded SignerInfo sequence exposed by
  the live WinTrust cryptographic message before and after M229's complete
  boundary.
- **Status:** Locally complete; publication withheld. Focused primary-source direction research, exact M229 baseline,
  protected-hash capture, live capability preflight, intentional architecture
  red, native implementation, complete message/count/size/failure-close cases,
  RFC-0213, security guidance, eight public registrations, focused and complete
  source/live/docs validation, supported-interpreter, rendering/profile/example,
  reproducible package/release, package-inventory, and findings-first review
  gates are complete. Terminating separator, guarded cleanup, DCO commit, and
  publication-safety gates pass. No real remote branch contains required M229,
  so no push, PR, or hosted allocation was created.
- **Base:** exact fully locally validated M229 DCO commit
  `e849c24aa5f742effbb30f9ea04a36d481598a38`, tree
  `1cced1d87b707920b8594152fe5d86c522e55535`, with sole parent exact M228.
- **Branch:** `release/m230-windows-source-commit-git-signer-info-binding`;
  exact containment made the local M229 branch redundant, so only local `main`
  and active M230 remain.

### M230 acceptance boundary

- Repeat M229's exact retained path/handle, no-UI, cache-only,
  explicit-no-revocation `WINTRUST_ACTION_GENERIC_VERIFY_V2` request.
- While each successful state is live, require a compatible provider prefix,
  positive raw message encoding, non-null cryptographic-message handle, and a
  positive bounded provider signer count.
- Query the message signer count into exactly one `DWORD`; require the same
  positive bounded count as the provider.
- Retrieve every encoded SignerInfo by exact zero-based message index through a
  successful size query and exact-size read. Reject empty, oversized, changed,
  or aggregate-overflowing results.
- Copy every complete value before state close. Bind raw encoding, both counts,
  every exact index and length, per-signer SHA-256 values, and one domain-
  separated sequence digest.
- Close provider state in `finally` after success, trust rejection, or any
  provider/message extraction failure. Report close failure when extraction
  otherwise succeeds.
- Require the complete immutable observation across M229's complete boundary.
- Preserve every M221-M229 evidence file byte-for-byte. Protect runtime,
  package, dependency, lock, workflow, fixture, example, script, and benchmark
  surfaces; add zero GitHub Actions job or hosted allocation.
- Keep SignerInfo parsing/validation, portable timestamp semantics, algorithm/
  attribute policy, timestamp-authority/signer/publisher authorization,
  ...
  DLL/native-loader identity, local-object-store trust, repository acquisition,
...
- Exact M229 history and a clean worktree were confirmed. The retained M59/M229
  group passes 38 tests in 3.33 seconds; static and 2026-09-03 strict governance
  each return zero findings.
- Current Microsoft provider-data, message-parameter, SignerInfo, and
  Authenticode guidance, Git for Windows 2.55.0.5, and SLSA 1.2 support one
  opaque execution-local encoded-signer sequence while parsing, authorization,
  timestamp semantics, and provenance remain separate.
- M230 was created from exact M229; redundant local M229 was removed. Protected
  M229/workflow/package hashes were captured and only local `main` plus active
  M230 remain.
- A live preflight observed raw encoding 65,537, one provider signer, one
  message signer, and one 7,209-byte encoded SignerInfo; its temporary probe was
  removed immediately.
- The intentional architecture red passed two protected/non-runtime checks and
  failed ten checks only because the planned probe, RFC/security decision, and
  public registrations were absent.
- The first native implementation exposed two static typing findings and one
  incorrect oversized-count fixture expectation; the correction changed no
  boundary. Both new Python files are format- and Ruff-clean; strict Pyright
  reports zero findings and all 28 live, detachment, digest-boundary, count,
  size, capacity, rejection, and failure-close tests pass in 2.04 seconds.
- RFC/security documentation and eight public registrations are present. Both
  Python files remain format/Ruff/strict-Pyright clean; all 40 architecture/live
  tests pass in 2.11 seconds and strict docs build in 3.88 seconds with only the
  known Material notice.
- The unchanged 46-package lock, 45-package frozen graphics environment, all
  648 Python files, Ruff, strict Pyright, strict docs, both governance modes,
  and whitespace pass. Architecture/integration passes 3,581 tests with 12
  expected skips in 239.09 seconds.
- Isolated explicit CPython 3.12.13, 3.13.13, and 3.14.5 environments each
  install 45 packages and pass 4,703 tests with 19 expected skips in 271.24,
  262.43, and 273.35 seconds.
- Ten real-wgpu tests pass in 6.83 seconds. Five-repeat base/graphics profiles
  emit two/three workloads and all eight profile tests pass in 2.39 seconds.
  Two Null and one wgpu Clockwork executions agree on state, draws, and sprites;
  Agent World Builder reproduces its state/capture/replay hashes and the current
  registered two-test selector passes in 0.63 seconds.
- Two initial builds are byte-identical: a 366,279-byte pure wheel at SHA-256
  `29fd3640d0dd47e57dd742ee35a5a58ffda961e8183d15516b12f7cda2f1d086`
  and a 2,720,018-byte source archive at SHA-256
  `65478582363d1f58186e82bc83b3af495e0ca75660499031e040c383244a84b3`.
  Installed smokes, two exact ten-artifact release stages, and package inventory
  pass.
- Findings-first review confirms exactly 16 intended paths and 1,385 additions
  plus one deletion, zero protected/runtime/workflow/package-surface change,
  zero public tool-identity/credential/local-path finding, absent retired
  metadata, clean whitespace/connectivity, neutral two-branch hygiene, and no
  remaining actionable correctness, security, architecture, compatibility,
  package, or CI-allocation issue. Six exact M230 generated scratch targets
  contain 28 entries pending guarded cleanup.
- Record-inclusive lock, all 648 Python files, Ruff, strict Pyright, strict docs,
  both governance modes, and whitespace pass; architecture/integration passes
  3,581/12 in 237.54 seconds.
- Two record-state builds preserve the 366,279-byte wheel and reproduce a
  2,720,603-byte source archive at SHA-256
  `357e13d45c17c883fb3f3baf736cfebedfb08019adc19b2744dd4b4f25bec50f`.
  Installed smokes, both release stages, release comparison, and 114-wheel/
  1,088-source inventory pass; all four M230 files are source-only.
- The final source separator keeps the unchanged lock and all 648 Python files
  clean under formatting, Ruff, and strict Pyright; strict docs, both
  governance modes, whitespace, and 3,581 architecture/integration tests with
  12 expected skips pass.
- Two final builds preserve the 366,279-byte wheel and reproduce a 2,720,840-
  byte source archive at SHA-256
  `a56cda2a8ee33332348312a4a47862e45e2e556c9dc6d0b14aa1bdfb258c79a9`.
  Installed smokes, both exact ten-artifact release stages, release comparison,
  and exact 114-wheel/1,088-source inventory pass.
- Final exact review confirms 16 intended paths and 1,441 then-current
  additions plus one deletion, zero protected-surface change, zero public
  tool-identity or delta credential/local-path finding, absent retired
  metadata, clean whitespace/connectivity, neutral two-branch hygiene, and no
  remaining actionable issue. Fourteen exact M230 generated scratch targets
  contain 80 entries pending guarded cleanup.
- The terminating source separator keeps all 648 Python files format-clean,
  Ruff and strict Pyright clean, dated strict governance at zero findings, and
  whitespace clean. The exact M59/M230 group passes 45 tests in 2.49 seconds
  through Python's module entry point; two earlier console-script invocations
  stopped at collection because importlib mode omitted the repository root.
- Guarded cleanup revalidated all 14 exact M230 scratch targets and 80 entries
  as repository-confined, ignored, untracked, and reparse-free, removed only
  that exact set, confirmed zero M230 target remains, and preserved all 56
  older `.tmp` targets.
- The post-cleanup no-artifact gate resolves the unchanged lock, keeps all 648
  Python files and strict docs clean, returns zero Ruff/Pyright/governance
  findings, passes the exact 45-test M59/M230 group, whitespace, and full
  connectivity, and confirms the unchanged M229 base plus exact 16-path scope.
- Initial DCO commit `619a611d5c62acbbc181d40ba8beb4a571ec4504`
  has exact tree `d2952fea69c68750a1656f0d2903b3ce0849559e`, sole parent
  exact M229, exact 16-path/1,482-addition/one-deletion scope, correct identity
  and DCO trailer, and a clean worktree. Only the factual three-record amend
  follows before hosted publication safety is evaluated.
- Amended DCO commit `dde93938772a7c54a0108451de8e78e65f57b518`
  has exact tree `52edbe67ad0cd3820ffdf594e856db1d84070815`, sole parent exact
  M229, exact 16-path/1,492-addition/one-deletion scope, correct identity/DCO,
  and a clean worktree. The terminating M59/M230 group passes 45 tests in 2.51
  seconds before this final factual amend.
- Fresh fetch and GitHub inventory find only hosted `main` at exact M99
  `5238941c77fbbbd0ff5fd72834d3bead66b2ed3e`; no real remote branch
  contains required M229, no PR is open, and merged PR #251 remains latest.
  Publication is therefore withheld without a push, PR, or workflow allocation.

### Remaining gates

- No local M230 gate remains. Publish only after the hosted prerequisite chain
  contains M229; do not expose the unpublished 131-commit stack merely to open
  a PR.

## M229 Windows source-commit Git countersigner-chain binding

- **Task:** Bind the complete bounded countersigner and countersigner-
  certificate sequences exposed by successful live WinTrust state before and
  after M228's complete boundary.
- **Status:** Locally complete; publication withheld. Focused direction
  research, exact M228 baseline, protected-hash
  capture, intentional architecture red, native implementation, complete
  indexed countersigner-chain failure/close cases, RFC-0212, security guidance,
  eight public registrations, registered focused source/live/docs validation,
  strengthened digest/certificate-limit review, whole-project static/docs/
  governance/behavior, supported CPython 3.12-3.14, and rendering/profile/
  example gates plus initial reproducible packaging/release and findings-first
  review are complete. Record-inclusive source/behavior/package/release
  separators and final source/behavior/package/release separators are complete;
  final exact review, terminating focused separator, and guarded cleanup are
  complete. The signed commit and ledger amend are valid. A fresh hosted gate
  found no remote branch containing required parent M228, so no push or PR was
  created.
- **Base:** exact fully locally validated M228 DCO commit
  `1ced72e243bae4b3322dfa3e03278b7a588430ab`, tree
  `eb23d15d02427b513f3dd1c46c46be1a5f5ff255`, with sole parent exact M227.
- **Branch:** `release/m229-windows-source-commit-git-countersigner-chain-binding`;
  exact containment made the local M228 branch redundant, so only local `main`
  and active M229 remain.

### M229 acceptance boundary

- Repeat M228's exact retained path/handle, no-UI, cache-only,
  explicit-no-revocation `WINTRUST_ACTION_GENERIC_VERIFY_V2` request.
- While each successful state is live, require provider data, the primary
  signer, and the complete positive bounded countersigner sequence by exact
  zero-based provider index.
- Require zero provider error, positive raw verification time, and a complete
  positive bounded provider-certificate sequence for every countersigner.
- Copy positive per-certificate, per-chain, and aggregate-bounded DER before
  state close. Bind raw signer type/error/time, every exact index and length,
  per-certificate/per-chain hashes, and one domain-separated sequence digest.
- Close provider state in `finally` after success, trust rejection, or any
  indexed extraction failure. Report close failure when extraction otherwise
  succeeds.
- Require the complete immutable observation across M228's complete boundary.
- Preserve every M221-M228 evidence file byte-for-byte. Protect runtime,
  package, dependency, lock, workflow, fixture, example, script, and benchmark
  surfaces; add zero GitHub Actions job or hosted allocation.
- Keep portable timestamp semantics, timestamp-authority/signer/publisher
  authorization, persisted identity, certificate pinning, rotation/recovery,
  timestamp-token/signing-time validation, revocation freshness, trust-store
  administration, DLL/native-loader identity, local-object-store trust,
  repository acquisition, source/build provenance, distinct principal,
  hostile/privileged behavior, independent host, criteria 6/7, cleanup
  authority, and Windows admission unresolved.

### Validation evidence so far

- Exact M228 history and a clean worktree were confirmed. The 27-test M228
  architecture/live baseline passes in 2.32 seconds; 2026-09-03 strict
  governance returns zero findings.
- Current Microsoft provider-signer/helper and Authenticode timestamp guidance,
  current Git for Windows release information, and SLSA 1.2 support one
  complete execution-local countersigner-chain observation while portable
  timestamp policy, authorization, and provenance remain separate. The
  installed Git reports valid signer and time-stamper certificates.
- M229 was created from exact M228; redundant local M228 was removed. Protected
  M228/workflow/package hashes were captured and only local `main` plus active
  M229 remain.
- The intentional architecture red passed two protected/non-runtime checks and
...
- Ruff formatting changed only the new integration file; focused Ruff and
  strict Pyright report zero findings. All 20 live, detachment, boundary-
  digest, provider-error, size, rejection, and failure-close tests pass in 2.58
  seconds.
- Registered format/Ruff/strict-Pyright checks pass; all 31 architecture/live
  tests pass in 2.68 seconds and strict docs build in 3.89 seconds. Findings-
  first review then added exact equal-byte boundary/order, null certificate-
  context, and per-chain DER-cap cases; all 33 strengthened focused tests pass
  in 3.02 seconds with clean Ruff and strict Pyright.
- The unchanged 46-package lock, 45-package frozen graphics environment, all
  646 Python files, Ruff, strict Pyright, strict docs, both governance modes,
  and whitespace pass. Architecture/integration passes 3,541 tests with 12
  expected skips in 231.83 seconds.
- Isolated explicit CPython 3.12.13, 3.13.13, and 3.14.5 environments each
  install 45 packages and pass 4,663 tests with 19 expected skips in 267.63,
  254.11, and 266.11 seconds. The first 3.13 sync selected the project-default
  3.12 interpreter before testing; explicit `--python 3.13` supplied the
  accepted result.
- Ten real-wgpu tests pass in 7.28 seconds. Five-repeat base/graphics profiles
  emit two/three workloads and all eight profile tests pass in 0.85 seconds.
  Two Null and one wgpu Clockwork executions agree on state, draws, and sprites;
  Agent World Builder reproduces its state/capture/replay hashes and the current
  registered two-test selector passes in 0.69 seconds.
- Two initial builds are byte-identical: a 366,178-byte pure wheel at SHA-256
  `b16f5b983432da8c2d0e061cfb32571252837e874f76f1777fac54bc18af275b`
  and a 2,707,051-byte source archive at SHA-256
  `74a8aea168a4c888db302b53695f460c56f80bd06ebaa680dc6ba4a82838ef82`.
  Installed smokes, two exact ten-artifact release stages, and package inventory
  pass.
- Findings-first review confirms exactly 16 intended paths and 1,470 additions,
...
- Record-inclusive lock, all 646 Python files, Ruff, strict Pyright, strict docs,
  both governance modes, and whitespace pass; architecture/integration passes
  3,541/12 in 238.15 seconds.
- Two record-state builds preserve the 366,178-byte wheel and reproduce a
  2,707,267-byte source archive at SHA-256
  `09455327d0df69c68cc34ca314b563a917bbba863b28f877d27f1ca93d4dde6f`.
  Installed smokes, both exact release stages, release comparison, and
  114-wheel/1,084-source inventory pass; all four M229 files are source-only.
- The final source separator keeps all 646 Python files format-clean, Ruff and
  ...
  3,541 passed and 12 expected skips in 235.89 seconds.
- Two final builds preserve the 366,178-byte wheel at SHA-256
  `b16f5b983432da8c2d0e061cfb32571252837e874f76f1777fac54bc18af275b`
  and reproduce a 2,707,530-byte source archive at SHA-256
  `2850d01d8254764f094985804f2f00b727f0e6ebaa0e961f7bfa0760c7589c9b`.
  ...
  114 wheel/1,084 source entries with all four M229 files source-only.
- Final review proves exactly 16 intended paths and 1,519 then-current
  additions, zero protected-surface change, zero public tool-identity or delta
  credential/local-path finding, absent retired metadata, neutral branches,
  clean whitespace/connectivity, and no remaining actionable finding. The
  terminating M59/M229 group passes 38 tests.
- Access-enabled audit and immediate exact-set revalidation removed only 34
  M229 targets containing 134,379 entries. Zero M229 scratch remains and all 56
  older targets remain.
- The final no-artifact source separator passes. The exact pre-commit audit
  confirms M228 HEAD/tree/parent, exactly 16 intended paths with 1,538
  additions and one deletion, eight protected hashes, neutral branch and
  configured identity, zero disclosure/credential/local-path/retired-metadata
  findings, zero M229 scratch, and clean whitespace/object connectivity.
- Initial DCO commit `e4de3e8d1f08a400a1ed20a1dfad76b7c2a6cf64`, tree
  `02c3e83014d8001211f52c42ca3834e46fccc53d`, has sole parent exact M228,
  exactly 16 paths, one sign-off, matching author/committer, a clean worktree,
  expected `0 130` divergence, zero M229 scratch, and clean whitespace/object
  connectivity.
- Ledger amend `892b34e57c7be954873707672a65786e8a254549`, tree
  `76e269b08fc19097144b89ed2fe4df5862f7f34f`, preserves every commit
  invariant. Fresh hosted inspection confirms origin/main exact M99, only real
  remote branch `origin/main`, no hosted M228 ancestry, zero open PRs, and
  latest PR #251 merged. Publication is withheld; no remote mutation or hosted
  allocation occurred.

### Remaining gates

- None for M229. Continue with the next separately bounded milestone from this
  exact locally validated lineage; retain publication withholding until its
  required hosted parent chain exists.
...
- **Task:** Retain the selected Git executable file across the complete M223
  observation and demonstrate the retainer's ordinary write/delete exclusion
  without mutating the proof file.
- **Status:** Direction research, protected-baseline capture, intentional
  architecture red, scoped implementation, ACL-sensitive proof correction,
  RFC-0207, security guidance, eight public registrations, focused and whole-
  project static/type/live tests, complete supported-Python validation,
  rendering/profiles/examples, and initial reproducible distribution/release
  rehearsal are complete. Findings-first review, record-inclusive validation,
  distribution/release rehearsal, guarded cleanup, final separators, local DCO
  commit verification, and hosted publication-safety gate are complete.
  Publication is withheld because hosted `main` lacks the M100-M223
  prerequisite stack.
- **Base:** exact fully locally validated M223 DCO commit
  `81603c8e6c567063f8a3d492fed3d5be9b1c4489`, tree
  `b5d83a896a6b3d4e9baa88b9e91aa85e9f45bade`, with sole parent exact M222.
- **Branch:** `release/m224-windows-source-commit-git-file-retention`;
  exact containment made the local M223 branch redundant, so only local
  `main` and active M224 remain.

### M224 acceptance boundary

- Perform exactly one real Git lookup, require its canonical absolute file,
  and open that file through one non-inheritable read-only handle with only
  `FILE_SHARE_READ` before the M223 observation.
- Snapshot normalized path, volume/file identity, positive bounded size, and
  SHA-256 before and after all 48 inherited Git reads; require a fresh shared
  post-close snapshot to remain identical.
- Exercise the identical retainer against its writable test source using only
  competing access opens. Require exact sharing refusal for write and
  delete/rename while retained, settlement after close, and unchanged bytes.
  Do not misclassify installed-Git ACL denial as share-mode evidence.
- Preserve every M223/M222/M221 evidence file byte-for-byte and retain all
  inherited no-lazy-fetch, direct-command, source, image, Job, token, access,
  participant, settlement, and close requirements.
- Protect runtime, package, dependency, lock, workflow, fixture, example,
  ...
  allocation.
- Keep executable authenticity/signing/publisher/ACL policy, actual child-
  process image, DLL/native-loader identity, local-object-store trust,
  repository acquisition, source/build provenance, distinct principal,
  hostile/privileged behavior, independent host, criteria 6/7, cleanup
  authority, and Windows admission unresolved.
...
- The clean M223 focused baseline passes 10 tests. Static and dated-strict
  governance each report zero findings.
- Current primary Windows, Python, SLSA, GitHub, NIST, and engineering-systems
  evidence supports a local retained-file slice while retaining provenance,
  authority, admission, and hosted-allocation gaps.
- The intentional architecture red passed two protected/non-runtime checks and
  failed exactly eight absent implementation/documentation/registration
  checks.
- The first live run found installed Git write access returns access denied
  before share evaluation. The corrected proof retains the real Git file
  across M223 and separately exercises the same retainer against its writable
  test source without mutation. Focused Ruff/Pyright and all 12 tests pass.
- The unchanged 46-package lock and 45-package graphics environment resolve.
  All 636 Python files are format-clean; Ruff and strict Pyright report zero
  findings; strict docs and both governance modes pass.
- Complete architecture/integration passes 3,432 tests with 12 expected skips.
  Isolated frozen CPython 3.12, 3.13, and 3.14 each pass 4,554 tests with 19
  expected skips.
- Ten real-wgpu tests, five-repeat base/graphics profiles, eight profile tests,
  two Null and one wgpu Clockwork runs, and Agent World Builder pass with the
  established deterministic identities.
- Two builds reproduce a 365,699-byte pure wheel at SHA-256
  `c932fbddb8cc8eb7a59cc1d911d17eac7afa45bcbafd0ebfaa89237055ccd283`
  and a 2,642,242-byte source archive at SHA-256
  `acbf53523e1adf1a8593a562afc988d22d4b8eef6d810fd5495932a8326f612a`.
  Installed wheel/scene smokes and two exact ten-artifact release stages pass.
- Findings-first review confirms exactly 16 intended paths and unchanged
  protected surfaces. Three public summaries were corrected so the separate
  access-only proof cannot be mistaken for a direct installed-Git ACL result.
  Added-line disclosure, credential, local-path, retired-metadata, whitespace,
  and full-connectivity checks pass.
- Record-inclusive lock/format/Ruff/Pyright, strict docs, both governance
  modes, whitespace, and the repeated complete architecture/integration
  boundary pass; the latter reports 3,432 tests with 12 skips.
- Record-state builds reproduce a 365,711-byte pure wheel at SHA-256
  `0499ac3998dc626345f58231900fb8a1f077c10fced1761f2a145ddac5c4f855`
  and a 2,646,129-byte source archive at SHA-256
  `1e61022979248562f594263eb2d9150d98b69cfceaed493301619b44d3046226`.
  Installed smokes, two exact release stages, and the 114-entry wheel/1,064-
  entry source inventory pass; each of the four new M224 files occurs once
  only in the source archive.
- The post-review separator passes. A corrected terminating audit proved
  exactly 19 M224 scratch targets containing 50,184 entries confined, ignored,
  untracked, and recursively reparse-free. Immediate exact-set revalidation
  removed only those targets and confirmed zero remains; older scratch was
  untouched.
- Final post-cleanup format/Ruff/Pyright, strict docs, dated-strict governance,
  whitespace, and complete architecture/integration pass; the complete suite
  reports 3,432 tests with 12 expected skips in 228.06 seconds.
- Two final builds preserve the 365,711-byte pure wheel at SHA-256
  `0499ac3998dc626345f58231900fb8a1f077c10fced1761f2a145ddac5c4f855`
  and reproduce a 2,647,038-byte source archive at SHA-256
  `be88403c36ae83b25a23f1f82caeb0d2472cea3be4734bded24b7be9528589b0`.
  Installed smokes, both exact release stages, and package inventories pass.
- A separate final-ledger audit proved exactly six M224 targets containing
  8,485 entries confined, ignored, untracked, and recursively reparse-free;
  immediate exact-set revalidation removed only them and confirmed zero M224
  scratch remains.
- The no-output whole-project format/Ruff/Pyright, dated-strict governance,
  whitespace, and zero-scratch separator passes.
- The pre-commit audit passes exact M223 history, 16-path allowlist, neutral
  branch/identity, expected `0 124` divergence, protected surfaces, disclosure/
  credential/local-path scans across 845 added lines, retired metadata,
  scratch, formatting/lint/type/whitespace, and full connectivity.
- Initial local DCO commit
  `424ad9f3dbf44ecf57a4cd9b2f72daf8242099d7`, tree
  `4978e0a10a05ba143001fe34038f7f25b78a9287`, has exact M223 as sole
  parent, 16 intended paths, one matching sign-off, matching maintainer
  identity, clean revision whitespace/worktree/connectivity, expected `0 125`
  divergence, and zero M224 scratch. This factual record will be incorporated
  by amendment.
- Amended local DCO commit
  `b8dab63ee5903e3ef17f6a6d0c15264b5e20b028`, tree
  `94af602c63e27e9d369c3e92d6ae48a6a2bec410`, retains exact M223 as
  sole parent, 16 intended paths, one matching sign-off, matching maintainer
  identity, clean revision whitespace/worktree/connectivity, expected `0 125`
  divergence, and zero M224 scratch. This factual record is incorporated by
  the final amendment.
- A fresh pruned fetch and direct remote-head query leave public `origin/main`
  at exact M99 `5238941c77fbbbd0ff5fd72834d3bead66b2ed3e`. M223 is absent,
  M99 is present, only remote `main` exists, no PR is open, and PR #251 remains
  latest. Publishing M224 would expose the absent M100-M223 stack, so no push,
  PR, hosted allocation, tag, release, or package publication occurs.

### Remaining gates

- Await integration of the prerequisite stack before any M224 publication.
...
- **Task:** Correct M221's fixed Git object reader so a missing object cannot
  trigger an implicit promisor-remote fetch while preserving the full M221
  contained source-commit boundary.
- **Status:** Direction research, test-first implementation, complete supported-
  Python validation, rendering/profiles/examples, reproducible distributions/
  releases, review, guarded cleanup, final ledger, and local DCO gates pass.
  Publication is withheld because hosted `main` lacks the M100-M221
  prerequisite stack.
- **Base:** exact fully locally validated M221 DCO commit
  `d09d8b561dbebe49af62a6762ef145bb30499845`, tree
  `c8847a1db0a823396d2b86a0d9cb5914b68015ad`, with sole parent exact M220.
- **Branch:**
  `release/m222-windows-source-commit-no-lazy-fetch-exclusion`; exact
  containment made the local M221 branch name redundant, so only local `main`
  and active M222 remain.

### M222 acceptance boundary

- Add the documented `--no-lazy-fetch` global option before `-C` and fixed
  `GIT_NO_LAZY_FETCH=1` after removing every ambient `GIT_*` value.
- Execute the complete M221 boundary and regress the exact command,
  environment, no-shell/input, timeout, pipe, and creation-flag shape.
- Preserve M221's exact fixed commit/tree/parent/path/blob and all inherited
  M220 source, handle, Job, token, image, access, participant, settlement, and
  close requirements.
- Prove the M221 reader changed by exactly those two lines while protecting
  runtime, package, dependency, lock, workflow, fixtures, examples, scripts,
  and benchmarks. Add zero GitHub Actions job or hosted allocation.
- Keep trusted Git/local-object-store, repository-acquisition, source/build
  provenance, import/native-loader, distinct-principal, hostile-process,
  independent-host, privileged-bypass, criteria 6/7, cleanup-authority, and
  Windows-admission gaps unresolved. Historical M221 evidence is not rewritten.

### Validation evidence so far

- The intentional architecture red passed two protected checks and failed the
  eight expected absent implementation/documentation/registration checks.
  After the exact two-line correction, regression, RFC-0205, security guide,
  and eight registrations, all 13 focused architecture/live tests pass.
- The unchanged 46-package lock resolves; 45 locked packages are installed;
  all 632 Python files are format-clean; Ruff and strict Pyright report zero
  findings. Static and dated-strict governance each return zero findings.
- Complete architecture/integration passes 3,410 tests with 12 expected skips.
  Two earlier foreground observations ended before a terminal result and are
  not counted; the detached exact rerun produced the accepted result.
- Isolated frozen CPython 3.12, 3.13, and 3.14 suites each pass 4,532 tests with
  19 expected skips and exact exit zero.
...
- Two builds reproduce a 365,482-byte pure wheel at SHA-256
  `48f75b2f9f1d6718407cfee996715ca123bc94c0ee469e7995381a8be86aacf9`
  and a 2,622,731-byte source archive at SHA-256
  ...
  pass. The 114-entry wheel contains zero M222 evidence/native file; the
  1,056-entry source archive contains each of four new M222 files once.
- Preliminary review confirms exactly 13 non-record paths, the M221 reader's
  exact two-line correction, unchanged protected surfaces, and no explicit
  development-tool identity or credential-like assignment. One initial scan
  command had a PowerShell quoting error and its corrected exact rerun passed.
- Record-inclusive format/Ruff/Pyright, 3,410/12-skip
  architecture/integration, strict docs, static/dated-strict governance, and
  whitespace pass. The exact 17-path allowlist, protected surfaces, retired-
  metadata absence, and 654-line disclosure/credential/local-path scan pass;
  no actionable review finding remains.
- Two record-state builds preserve the exact wheel and reproduce a
  2,625,704-byte source archive at SHA-256
  `0a51570adcc57d2ddd1f763b8b5012a6e7d6adfd02b07717ae1604e35848c232`.
  Installed smokes and both exact ten-artifact release rehearsals pass.
- The post-review separator passes whole-project format/Ruff/Pyright, all 13
  focused tests, strict docs, dated-strict governance, and whitespace.
- A corrected guarded audit proved exactly 35 M222 targets and 62,495 entries
  confined, ignored, untracked, and recursively reparse-free. Immediate exact-
  set revalidation removed only those targets and confirmed zero remains;
  older scratch was untouched.
- Final post-cleanup format/Ruff/Pyright, 3,410/12-skip
  architecture/integration, strict docs, dated-strict governance, and
  whitespace pass. Two final builds preserve the exact wheel and reproduce a
  2,626,600-byte source archive at SHA-256
...
- Nine exact final-ledger targets and 8,474 entries were separately proven
  confined, ignored, untracked, and recursively reparse-free, removed exactly,
  and confirmed absent; older scratch remains untouched.
- The no-output separator passes whole-project format/Ruff/Pyright,
  dated-strict governance, whitespace, and zero-scratch checks.
- The pre-commit audit passes exact M221 history, 17-path allowlist, neutral
  branch/identity, `0 122` divergence, protected-surface, disclosure,
  credential, local-path, retired-metadata, scratch, whitespace, and full Git
  connectivity gates.
- Initial DCO commit `703ee11ea7be8a3793dc47ed22e6b568390d3189`, tree
  ...
  zero M222 scratch. This factual record will be incorporated by amendment.
- Amended DCO commit `bd184eeebd480aa8b4bc33f82a9dc95747e7c522`,
  tree `9c7a97c390da0b660b1ef63be6af7c45fc828dae`, retains exact M221 as
  sole parent, 17 intended paths, one matching sign-off, consistent identity,
  a clean worktree/connectivity, expected `0 123` divergence, and zero scratch.
- Final pre-publication commit
  `afb7f564a556441276b66f42e9a8927ca93e0fc2`, tree
  `36602be3264531920ad9f5d930a19dbbd7df7bf2`, retains the same exact
  17-path/DCO/identity/parent/cleanliness/connectivity boundary.
- A fresh pruned fetch and direct remote-head query leave public `origin/main`
  at exact M99 `5238941c77fbbbd0ff5fd72834d3bead66b2ed3e`.
  M221 is absent, only remote `main` exists, no PR is open, and PR #251 remains
  latest. Publishing M222 would expose the absent M100-M221 stack, so no push,
  PR, hosted allocation, tag, release, or package publication occurs.

### Remaining gates

- Await integration of the prerequisite stack before any M222 publication.
- Recheck hosted ancestry, identity, branch hygiene, DCO, and CI scope before
  any future push or PR.
- Recheck hosted ancestry before push or PR. Publication remains forbidden if
  hosted `main` lacks the M100-M221 prerequisite stack.
...
- **Task:** Compose exact `-X disable_remote_debug` startup with the frozen
  M212-M216 Windows retained process/source boundary without attempting remote
  attachment, injection, collection, cleanup, or Windows admission.
- **Status:** Direction research, implementation, complete local validation,
  findings-first review, record-state packaging/release rehearsal, guarded
  cleanup, closeout, and the local DCO commit are complete. Publication is
  withheld because hosted `main` lacks the M100-M216 prerequisite stack.
- **Base:** exact fully locally validated M216 DCO commit
  `c10fd5d6ac95f846dd601b30290aadefa8bf2e4a`, tree
  `501ada51735810995b7e81af0d8c6a5a74ecde4a`, with sole parent exact M215.
- **Branch:** `release/m217-windows-remote-debug-exclusion-probe`; exact
  containment made the local M216 branch name redundant, so only local `main`
  and active M217 remain.

### M217 acceptance boundary

- Retain M215's canonical pipe-name validator and compose exact fixed direct
  `pythonw.exe -I -B -X disable_remote_debug - <pipe>` only for participant
  process creation.
- Preserve M216's three-phase write/delete access refusal, retained token,
  session/DACL, executable-image, source-stability, release, settlement,
  post-settlement access, and zero-owned-handle boundary.
- Treat the option as remote-debug exclusion evidence only on Python 3.14;
  Python 3.12/3.13 establish command and lifecycle compatibility only.
- Do not attempt remote attachment, code injection, process-memory access,
  source mutation, namespace mutation, or an enabled/disabled attack pair.
- Protect exact M216 evidence plus every runtime, package, fixture, example,
  script, benchmark, workflow, dependency, lock, metadata, and version surface.
- Keep source provenance, imported-module binding, hostile-process behavior,
  distinct-principal and independent-host proof, collection, cleanup, criteria
  6/7, and Windows admission unresolved. Add zero GitHub Actions jobs or hosted
  allocation.

### Validation evidence so far

- The exact M216 focused handoff passed 24 tests; static and dated-strict
  governance passed with zero findings before the branch was created.
- The intentional architecture red recorded eight absent-artifact/registration
  failures and two frozen-boundary passes. The first complete live run exposed
  recursion when the scoped composer consulted its patched module binding; a
  retained reference to M215's original validator corrected the composition.
- The exact M217 architecture/live group passes all 19 tests on CPython 3.12.
  Focused Ruff and strict Pyright report zero findings; strict docs build with
  only the known Material notice; whitespace passes.
- Current primary-source research supports the exact 3.14 exclusion spelling,
  narrow 3.12/3.13 compatibility claim, unchanged Windows handle/share
  boundary, and zero added public CI allocation.
- All 620 Python files are format-clean; Ruff and strict Pyright report zero
  findings; architecture plus the live M217 probe passes 2,294 tests with one
  inherited capability skip; strict docs, both governance modes, and
  whitespace pass.
- Complete isolated all-group graphics suites pass 4,453 tests with 19 skips
  on each CPython 3.12, 3.13, and 3.14. Ten real-wgpu tests, both one-repeat
  profile contracts, eight profile tests, two Null Clockwork repeats, wgpu
  Clockwork, and Agent World Builder pass.
- Two builds reproduce a 364,868-byte pure wheel at SHA-256
  ...
  rehearsals pass. The 114-entry wheel contains no M217 file; all four new
  implementation/decision files occur once in the 1,034-entry source archive.
- Findings-first review confirms exactly 16 intended paths, no protected
  runtime/package/fixture/CI/dependency/lock change, exact restoration of the
  frozen composer, no attachment/injection/mutation operation, no authority
  increase, and no actionable finding.
- Record-inclusive 620-file format/Ruff/Pyright, 2,294-test architecture/live,
  strict docs, both governance modes, and whitespace gates pass.
- Record-state builds reproduce the unchanged wheel and identical
  2,574,415-byte source archives at SHA-256
  `e803d66d160ead455d6c3cdd78d37b12b8b32c669f25d3d0c89b58347891f9c1`;
  installed smokes and both exact release rehearsals pass with the unchanged
  source-only package boundary.
- An access-enabled audit proved all 21 initial M217 generated targets and
  48,902 descendants repository-confined, ignored, untracked, and recursively
  reparse-free before exact removal. The regenerated docs directory and its
  760 descendants passed the same independent audit before removal. Older
  outputs and all inaccessible M212 roots were untouched.
- Post-cleanup 620-file static/type gates, the exact 19-test M217 boundary,
  strict docs, dated-strict governance, and whitespace pass. DCO commit-object
  and Git publication safety remain pending.
- Pre-commit audit confirms exact M216 history/tree/parent, exactly 16 intended
  paths, only local `main` plus active neutral M217, expected `0 117`
  divergence, configured maintainer identity, zero protected-surface change,
  zero M217 scratch, absent retired root metadata, clean whitespace, and
  connected Git objects.
- Initial DCO commit `d12f386c060aa44e52546a02bbb1ab5c4dcac218`, tree
  `cebc30dd21f72df8e441ff3c16ea12255e4f444f`, has sole parent exact M216,
  exactly 16 paths, one matching sign-off, consistent maintainer identity,
  clean revision/worktree/connectivity, and expected `0 118` divergence. This
  factual record is incorporated by the closeout amendment.
- Fresh pruned fetch and direct remote-head lookup leave public `origin/main`
  at exact M99 `5238941c77fbbbd0ff5fd72834d3bead66b2ed3e`; M216 is absent,
  M99 is present, only remote `main` exists, and divergence is `0 118`.
  Authentication is valid; the repository is public, unarchived, and defaults
  to `main`; PR #251 remains latest. Publishing M217 would expose absent
  M100-M216 prerequisites, so no push, PR, hosted allocation, tag, release, or
  package publication occurs.
