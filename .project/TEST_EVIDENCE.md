...
| --- | ---: | --- |
| Primary-source direction scan | Advisory | PKWARE APPNOTE 6.3.10 defines exact extra-field ID `0x0001` as ZIP64 extended information carrying alternate size, offset, and disk-start values. Current CPython substitutes those values when corresponding central-directory fields contain sentinels. The selected response is exact field-ID preflight, not a broad non-empty-extra rule or raw ZIP64 parser. |
