"""Install one wheel and verify explicit post-realization cache population."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    uv = shutil.which("uv")
    if uv is None:
        raise RuntimeError("uv is required for the isolated asset-cache-population smoke")
    ...
    project_document: dict[str, object] = {
        ...
        "world_id": "installed-asset-cache-population",
    ...
    scene: dict[str, object] = {
        ...
        "scene_id": "installed-asset-cache-population-scene",
        "entities": [],
        "dependencies": ["asset://data/item.json"],
    }
    sources: dict[str, object] = {
        "$schema": "ludoweave.source-manifest/1",
        "manifest_id": "installed-asset-cache-population-sources",
    ...
    with tempfile.TemporaryDirectory(
        prefix="ludoweave-asset-cache-population-wheel-smoke-",
        dir=local_temp,
    ) as temp_name:
        ...
        populated_cache = _files(cache)
        second = cast(dict[str, object], json.loads(_run(command, cwd=temp_root).stdout))
        unchanged = _files(project) == before_project and _files(cache) == populated_cache

    if (
        first.get("$schema") != "ludoweave.asset-cache-population/1"
        or (first.get("hits"), first.get("decoded")) != (0, 1)
        or (first.get("published"), first.get("reused")) != (1, 0)
        or (second.get("hits"), second.get("decoded")) != (1, 0)
        or (second.get("published"), second.get("reused")) != (0, 1)
        or not unchanged
    ):
        raise RuntimeError(
            "installed asset cache population was invalid: "
            f"first={first!r}, second={second!r}, unchanged={unchanged!r}"
        )
    print(
        json.dumps(
            {
                "population_protocol": second["$schema"],
                "schema": "ludoweave.asset-cache-population-wheel-smoke/1",
                "status": "pass",
            },
            sort_keys=True,
        )
