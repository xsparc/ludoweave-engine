"""Install one wheel and preview one saved cache fingerprint offline."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    uv = shutil.which("uv")
    if uv is None:
        raise RuntimeError("uv is required for the isolated fingerprint-record preview smoke")
    ...
    project_document: dict[str, object] = {
        ...
        "world_id": "installed-cache-fingerprint-record-preview",
    ...
    scene: dict[str, object] = {
        ...
        "scene_id": "installed-cache-fingerprint-record-preview-scene",
        "entities": [],
        "dependencies": ["asset://data/item.json"],
    }
    sources: dict[str, object] = {
        "$schema": "ludoweave.source-manifest/1",
        "manifest_id": "installed-cache-fingerprint-record-preview-sources",
    ...
    with tempfile.TemporaryDirectory(
        prefix="ludoweave-cache-fingerprint-record-preview-wheel-smoke-",
        dir=local_temp,
    ) as temp_name:
        ...
        orphan = b"installed fingerprint-record preview orphan"
    ...
    if (
        ...
        raise RuntimeError(
            "installed fingerprint-record preview was invalid: "
            f"report={report!r}, unchanged={unchanged!r}, cache_absent={cache_absent!r}"
        )
    print(
        json.dumps(
            {
                "schema": "ludoweave.asset-cache-fingerprint-record-preview-wheel-smoke/1",
                "status": "pass",
