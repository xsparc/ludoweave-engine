"""Install one wheel and verify deterministic cache-observation fingerprinting."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    uv = shutil.which("uv")
    if uv is None:
        raise RuntimeError("uv is required for the isolated cache-fingerprint smoke")
    ...
    project_document: dict[str, object] = {
        ...
        "world_id": "installed-cache-fingerprint",
    ...
    scene: dict[str, object] = {
        ...
        "scene_id": "installed-cache-fingerprint-scene",
        "entities": [],
        "dependencies": ["asset://data/item.json"],
    }
    sources: dict[str, object] = {
        "$schema": "ludoweave.source-manifest/1",
        "manifest_id": "installed-cache-fingerprint-sources",
    ...
    with tempfile.TemporaryDirectory(
        prefix="ludoweave-cache-fingerprint-wheel-smoke-",
        dir=local_temp,
    ) as temp_name:
        ...
        orphan = b"installed fingerprint orphan"
        ...
        first_result = _run(command, cwd=temp_root)
        second_result = _run(command, cwd=temp_root)
        report = cast(dict[str, object], json.loads(first_result.stdout))
        inventory = cast(dict[str, object], report.get("inventory"))
        unchanged = _files(project) == before_project and _files(cache) == before_cache

    if (
        first_result.stdout != second_result.stdout
        or report.get("$schema") != "ludoweave.asset-cache-fingerprint/1"
        or not str(report.get("observation_sha256", "")).startswith("sha256:")
        or inventory.get("$schema") != "ludoweave.asset-cache-inventory/1"
        ...
        raise RuntimeError(
            "installed asset cache fingerprint was invalid: "
    ...
    print(
        json.dumps(
            {
                "schema": "ludoweave.asset-cache-fingerprint-wheel-smoke/1",
                "status": "pass",
                "fingerprint_protocol": report["$schema"],
            },
            sort_keys=True,
        )
    )
