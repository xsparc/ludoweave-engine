"""Install one wheel and preview unreferenced cache blobs without mutation."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    uv = shutil.which("uv")
    if uv is None:
        raise RuntimeError("uv is required for the isolated unreferenced-preview smoke")
    ...
    project_document: dict[str, object] = {
        ...
        "world_id": "installed-cache-unreferenced-preview",
    ...
    scene: dict[str, object] = {
        ...
        "scene_id": "installed-cache-unreferenced-preview-scene",
        "entities": [],
        "dependencies": ["asset://data/item.json"],
    }
    sources: dict[str, object] = {
        "$schema": "ludoweave.source-manifest/1",
        "manifest_id": "installed-cache-unreferenced-preview-sources",
    ...
    with tempfile.TemporaryDirectory(
        prefix="ludoweave-cache-unreferenced-preview-wheel-smoke-",
        dir=local_temp,
    ) as temp_name:
        ...
        orphan = b"installed unreferenced-preview orphan"
        ...
        command = [*prefix, "asset-cache-unreferenced-preview", *cache_arguments]
        ...
        shutil.rmtree(cache)
        absent = _run(command, cwd=temp_root)
        absent_report = cast(dict[str, object], json.loads(absent.stdout))
        absent_cache = not cache.exists()

    if (
        first.stdout != second.stdout
        or report.get("$schema") != "ludoweave.asset-cache-unreferenced-preview/1"
        or report.get("status") != "observed"
        or report.get("fingerprint_protocol") != "ludoweave.asset-cache-fingerprint/1"
        or report.get("inventory_protocol") != "ludoweave.asset-cache-inventory/1"
        or report.get("unreferenced_blobs") != 1
        or report.get("unreferenced_blob_bytes") != len(orphan)
        ...
        or not unchanged
        or absent_report.get("unreferenced_blobs") != 0
        or absent_report.get("unreferenced_blob_bytes") != 0
        or not absent_cache
    ):
        raise RuntimeError(
            "installed cache unreferenced preview was invalid: "
            f"report={report!r}, absent={absent_report!r}, unchanged={unchanged!r}"
        )
    print(
        json.dumps(
            {
                "schema": "ludoweave.asset-cache-unreferenced-preview-wheel-smoke/1",
                "status": "pass",
                "preview_protocol": report["$schema"],
            },
            sort_keys=True,
        )
    )
