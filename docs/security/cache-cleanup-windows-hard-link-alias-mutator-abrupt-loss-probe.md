# Windows hard-link alias mutator abrupt-loss probe

- **Status:** Accepted current-host bounded capability evidence; Windows is not admitted
- **Milestone:** M187
- **Date:** 2026-08-29
- **Baseline:** M186's independent hard-link alias mutator ABA boundary

## Decision

Retain one Windows-only, test-only NTFS observation showing that abrupt process
loss after M186's mutator child emits `deleted`, but before it receives the
recreate token, leaves the peer alias absent. The original retains its identity,
bytes, and one-link count while M181's matching guardian remains live and
continues protecting the exact name. Treat this as three-process,
same-principal negative recovery evidence. There is no automatic rollback or
recovery, Windows is not admitted, and no cleanup authority is created.
...
4. starts M186's unchanged fixed mutator child and requires its exact
   `deleted` event with both children live;
5. before any recreate token, requires alias absence, unchanged original
   identity and bytes, link count one, and exclusive byte-range availability;
...
7. requires the guardian still live, the alias still absent, identity and bytes
   unchanged, link count still one, range availability, and exact-name rename
   refusal;
8. releases the guardian exactly, renames the original successfully, and
   verifies the displaced identity, one-link count, bytes, process/stream
   closure, and complete native/range ownership cleanup.

The probe uses no retry or sleep. It sends no recreate or close token to the
mutator after `deleted`. The fixture remains byte-for-byte M186's fixed-name,
argument-free, shell-free child. Abrupt termination is confined to a controlled
test child whose exact phase is known; it is not a production cleanup action.

## Security consequence

The observed one-link state persists after the mutation actor is reaped. The
guardian's live no-delete-share handle still refuses rename of the original
name, but does not restore the removed peer directory entry. Exact-name
...
cross-principal, unrelated-session, hostile-process, simultaneous-race,
crash-consistency, or power-loss evidence.
...
- [RFC-0170](../rfcs/0170-probe-windows-hard-link-alias-mutator-abrupt-loss.md)
