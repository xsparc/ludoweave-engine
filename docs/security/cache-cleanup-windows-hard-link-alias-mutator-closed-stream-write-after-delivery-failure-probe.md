# Windows hard-link alias mutator closed-stream write after delivery failure probe

- **Status:** Accepted current-host bounded capability evidence; Windows is not admitted
- **Milestone:** M198
- **Date:** 2026-08-30
- **Baseline:** M197's closed-stream flush disposition boundary

## Decision

Retain one Windows-only, test-only NTFS observation showing local stream
disposition after M197's closed-stream flush sequence. M195's first `close()`
remains the delivery attempt: it raises generic `OSError` and leaves the parent
stream closed. M196's second `close()` returns `None`. M197's one `flush()`
raises generic `ValueError`. M198 then calls one `write(b"!")` on that closed
concrete stream, requires generic `ValueError`, and requires the stream still
closed. This does not establish native-call suppression, a second native write,
retry, or acknowledgement from the child. The peer alias retains the original
identity, bytes, and two-link count while M181's matching guardian remains
live. This controlled test-only stream condition is not a production cleanup
action, and Windows is not admitted.
...
6. delegates M197's exact invalid settlement, late `!` buffer acceptance,
   ...
   to the byte-for-byte protected M197 helper;
7. requires the stream already closed, calls one `write(b"!")`, requires generic
   ...
   identity, bytes, count, and ranges unchanged, and exact-name rename still
   refused; and
8. releases the guardian exactly, renames the original successfully, and
   verifies displaced and alias identities, two-link counts, bytes, process and
   stream closure, and complete native and range ownership cleanup.

The probe uses no retry or sleep, no closed-stream operation beyond the one
`write(b"!")` added after M197, and no `communicate()`. It reuses M197's helper,
M186's byte-for-byte fixed-name, argument-free, shell-free child, and M193's
bounded-output ordering.

## Security consequence

The first `close()` error establishes failed pending-byte delivery for the fixed
fixture. The second `close()` and later `flush()` establish only local closed-
stream disposition. The later generic `ValueError` establishes only how one
concrete closed stream responds to one `write(b"!")` call. It does not prove
whether another native write was attempted, restore the lost byte, or establish
peer receipt. No acknowledgement is created. The observed two-link state
persists and the guardian's live no-delete-share handle still refuses rename of
the original name.
...
stream-operation, unbounded-output, duplicated or inherited-writer, cross-
...
framing, bounded I/O, acknowledgements and receipts, link enumeration and
policy, use-time identity and count revalidation, durable intent and generation
provenance, quarantine, idempotency, rollback and reconciliation, and typed
...
explicit framing, bounded I/O, acknowledged delivery, trusted root ownership,
a hard-link policy, use-time identity and link-count revalidation, durable
generation and recovery state, typed receipts, cross-principal adversarial
evidence, and independent-host proof.
...
- [RFC-0181](../rfcs/0181-probe-windows-hard-link-alias-mutator-closed-stream-write-after-delivery-failure.md)
