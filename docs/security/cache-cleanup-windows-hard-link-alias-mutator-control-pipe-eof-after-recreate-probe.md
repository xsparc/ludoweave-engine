# Windows hard-link alias mutator control-pipe EOF after recreation probe

- **Status:** Accepted current-host bounded capability evidence; Windows is not admitted
- **Milestone:** M189
- **Date:** 2026-08-29
- **Baseline:** M188's abrupt-loss-after-recreation boundary

## Decision

Retain one Windows-only, test-only NTFS observation showing that control-pipe
EOF after M186's mutator child emits `recreated`, but before it receives the
...
6. writes no further byte, closes only the parent `Popen.stdin`, and waits with
   M186's fixed bound for existing fixture exit 5, stdout EOF, and empty stderr;
...
name, argument-free, shell-free child. The EOF path is a controlled test-only
protocol condition, not a production cleanup action.

## Security consequence

The observed two-link state persists after the mutation actor settles through
control-pipe EOF. The guardian's live no-delete-share handle still refuses
rename of the original name, but it neither owns nor rolls back the peer
directory entry. Exact-name exclusion is therefore neither root-confined
...
abrupt process termination, cross-principal, duplicated/inherited-writer,
...
- [RFC-0172](../rfcs/0172-probe-windows-hard-link-alias-mutator-control-pipe-eof-after-recreate.md)
