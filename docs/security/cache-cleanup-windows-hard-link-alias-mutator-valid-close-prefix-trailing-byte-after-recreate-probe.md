# Windows hard-link alias mutator valid close prefix with trailing byte after recreation probe

- **Status:** Accepted current-host bounded capability evidence; Windows is not admitted
- **Milestone:** M191
- **Date:** 2026-08-29
- **Baseline:** M190's invalid control token after recreation boundary

## Decision

Retain one Windows-only, test-only NTFS observation showing that a fixed valid
close prefix with one trailing invalid byte after recreation is accepted by
M186's unchanged fixture. The child emits exact `closed` and exits 0 even
though the single flushed write contains `!?`. This leaves the peer alias present
with the original identity, bytes, and two-link count while M181's matching
guardian remains live and continues protecting the exact name. Treat this as
three-process, same-principal byte-prefix acceptance evidence. It is not
...
6. writes `!?` exactly once, requires the buffered write to accept both bytes,
   flushes it, requires exact `closed` while the parent writer remains open,
   then closes the writer and waits with M186's fixed bound for exit 0, stdout
...
The unchanged child consumes the valid first byte, emits its normal close
acknowledgement, and exits without rejecting the trailing invalid byte. The
observed two-link state persists and the guardian's live no-delete-share handle
still refuses rename of the original name. This proves only one fixture's
fixed byte-prefix acceptance; it does not supply framing, ownership, rollback,
or recovery.
...
- [RFC-0174](../rfcs/0174-probe-windows-hard-link-alias-mutator-valid-close-prefix-trailing-byte-after-recreate.md)
