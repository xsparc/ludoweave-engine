# Windows hard-link alias mutator late valid-close delivery failure after invalid settlement probe

- **Status:** Accepted current-host bounded capability evidence; Windows is not admitted
- **Milestone:** M194
- **Date:** 2026-08-29
- **Baseline:** M193's invalid-prefix open-writer settlement boundary
...
child has already settled, while delivery fails on flush. Buffer acceptance is
not peer receipt. The exact Python exception subtype and numeric translation
are deliberately outside the boundary: require only generic `OSError`, with no
exact Python exception code. The child emits no `closed` event, and the peer
alias retains the original identity, bytes, and two-link count while M181's
matching guardian remains live. This controlled test-only protocol condition
is not a production cleanup action, and Windows is not admitted.
...
7. after the child has already settled, writes one late valid close byte,
   requires the parent buffer to accept one byte, and requires delivery to fail
   on flush with generic `OSError`, without an exact code assertion;
8. closes the writer best-effort and requires it closed; then requires the
   guardian live, alias present, shared identity/bytes/count/ranges unchanged,
   and exact-name rename still refused; and
...
The flush failure establishes that the byte was not deliverable after the
child's read handle closed. Neither result is an acknowledgement, receipt, or
proof of child execution. The observed two-link state persists and the
...
- [RFC-0177](../rfcs/0177-probe-windows-hard-link-alias-mutator-late-valid-close-delivery-failure-after-invalid-settlement.md)
