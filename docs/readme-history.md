# Historical README reference

This page preserves the detailed README at M239 (commit
`89ca17cab106b21643ac2fc7147b599dab7112e7`). Milestone status statements below
are historical, not current release or support claims. M239 was squash-merged
after all three jobs in run 34290673055 passed. For onboarding, use the
[project README](https://github.com/xsparc/ludoweave-engine/blob/main/README.md); for validation, use the
[quality guide](quality.md). New milestone narratives belong in their dedicated
guides and the roadmap, not the project README.

Replay-owned input is available through experimental `ludoweave.app.replay.InputReplay`.
See the [input replay guide](input-replay.md) for separate-process recording
...
on Debian/Ubuntu). See the [audio contract](rfcs/0220-optional-blocking-audio.md).
...
[portable Git source-probe fixture](rfcs/0219-portable-git-source-probe-fixture.md)
...
against pinned historical fixture objects, not proof of current checkout provenance.
...
> model](security/cache-cleanup-threat-model.md) covering filesystem races,
...
> [platform-capability
> decision](security/cache-cleanup-platform-capability-decision.md) and
> requires real-host adapter evidence before any platform is admitted.
> M149 adds one test-only [Windows capability
> probe](security/cache-cleanup-windows-capability-probe.md). It exercises
...
> probe](security/cache-cleanup-windows-junction-probe.md). It executes a
...
> probe](security/cache-cleanup-windows-share-delete-exclusion-probe.md).
...
> probe](security/cache-cleanup-windows-native-sharing-violation-probe.md).
...
> probe](security/cache-cleanup-windows-control-pipe-eof-probe.md).
...
> probe](security/cache-cleanup-windows-invalid-control-token-probe.md).
...
> probe](security/cache-cleanup-windows-broken-control-pipe-probe.md).
...
> probe](security/cache-cleanup-windows-live-wait-timeout-probe.md).
...
> probe](security/cache-cleanup-windows-duplicated-handle-probe.md). A
...
> probe](security/cache-cleanup-windows-inherited-handle-probe.md). A
...
> probe](security/cache-cleanup-windows-inherited-launch-failure-probe.md).
...
> probe](security/cache-cleanup-windows-inherited-restore-failure-probe.md).
...
> probe](security/cache-cleanup-windows-descendant-non-exclusion-probe.md).
...
> probe](security/cache-cleanup-windows-cooperative-lock-probe.md). Two
...
> probe](security/cache-cleanup-windows-guardian-abrupt-handoff-probe.md).
...
> recreation probe](security/cache-cleanup-windows-hard-link-alias-mutator-control-pipe-eof-after-recreate-probe.md).
...
> after recreation probe](security/cache-cleanup-windows-hard-link-alias-mutator-invalid-control-token-after-recreate-probe.md).
...
> M191 adds one test-only [Windows hard-link alias mutator valid close prefix
> with trailing byte after recreation probe](security/cache-cleanup-windows-hard-link-alias-mutator-valid-close-prefix-trailing-byte-after-recreate-probe.md).
> Writing and flushing fixed `!?` produces the normal close acknowledgement while
> leaving the alias present; this is bounded byte-prefix acceptance evidence.
> M192 adds one test-only [Windows hard-link alias mutator invalid prefix with
> valid close suffix after recreation probe](security/cache-cleanup-windows-hard-link-alias-mutator-invalid-prefix-valid-close-suffix-after-recreate-probe.md).
> Writing and flushing fixed `?!` produces no close acknowledgement and exit 5
> while leaving the alias present; this is bounded leading-byte rejection evidence.
> M193 adds one test-only [Windows hard-link alias mutator invalid-prefix open-
> writer settlement after recreation probe](security/cache-cleanup-windows-hard-link-alias-mutator-invalid-prefix-valid-close-suffix-open-writer-settlement-after-recreate-probe.md).
...
> delivery-failure probe](security/cache-cleanup-windows-hard-link-alias-mutator-late-valid-close-delivery-failure-after-invalid-settlement-probe.md).
> A late byte can enter the parent buffer after child exit, but its flush fails;
> local buffer acceptance is not peer receipt.
> M195 adds one test-only [Windows hard-link alias mutator buffered-close
> delivery-failure probe](security/cache-cleanup-windows-hard-link-alias-mutator-buffered-close-delivery-failure-after-invalid-settlement-probe.md).
...
> close after delivery-failure probe](security/cache-cleanup-windows-hard-link-alias-mutator-repeated-buffered-close-after-delivery-failure-probe.md).
...
> M197 adds one test-only [Windows hard-link alias mutator closed-stream flush
> after delivery-failure probe](security/cache-cleanup-windows-hard-link-alias-mutator-closed-stream-flush-after-delivery-failure-probe.md).
...
> after delivery-failure probe](security/cache-cleanup-windows-hard-link-alias-mutator-closed-stream-write-after-delivery-failure-probe.md).
...
> refresh](security/cache-cleanup-windows-readiness-refresh.md) over the
...
> admitted.
> M205 adopts a [Windows cache-cleanup cross-principal validation
> contract](security/windows-cache-cleanup-cross-principal-validation-contract.md).
> A qualifying future run must use a genuinely distinct, independently
> authenticated untrusted principal; unrelated process/session topologies;
...
> M208 adds a source-only [Windows independent-host evidence
> validator](security/windows-cache-cleanup-independent-host-evidence-validator.md).
> It validates one bounded artifact against a separately validated and digest-
> bound M206 companion, but the reviewed fixture is entirely `not_run`, no
> qualifying host execution is claimed, and no hosted allocation is added.
> M209 accepts a [Windows independent-host collection-authority
...
> hosted check.
> M215 adds a test-only [Windows retained launch-source binding
> probe](security/windows-cache-cleanup-retained-launch-source-binding-probe.md).
> The child reads the retained participant source from isolated inherited
...
> M220 adds a test-only [Windows contained source-access source-binding
> probe](security/windows-cache-cleanup-contained-source-access-source-binding-probe.md).
> It retains a fixed contender source before child creation, executes those
...
> M222 adds a corrective test-only [Windows contained source-access
...
> It applies both documented Git lazy-fetch exclusions to every M221 object
> read and rejects ambient Git overrides. It adds no runtime, cleanup,
> admission, workflow, or hosted check.
> M223 adds a test-only [Windows contained source-access source-commit Git
...
> M228 adds a test-only [Windows contained source-access source-commit Git
...
> It copies and hashes every bounded provider certificate by exact provider
> index before and after M227. Portable chain semantics, signer authorization,
> certificate pinning, revocation freshness, native-loader identity,
> provenance, cleanup, admission, workflow, and hosted checks remain unchanged.

> M229 adds a test-only [Windows contained source-access source-commit Git
...
> It copies and hashes every bounded provider-certificate chain for every exact
> countersigner index before and after M228. Portable timestamp semantics,
> timestamp-authority or signer authorization, certificate pinning, revocation
...
> M231 adds a test-only [Windows contained source-access source-commit Git
...
> certificate DER to equal the primary provider certificate before and after
> M230. Publisher authorization, certificate pinning, revocation freshness,
> native-loader identity, provenance, cleanup, admission, workflow, and hosted
> checks remain unchanged.

> M232 adds a test-only [Windows contained source-access source-commit Git
...
> It retrieves every bounded exact message signer certificate selector and
> requires its issuer and serial number to equal the verified and same-index
> provider certificates before and after M231. Signer or publisher
> authorization, persistent identity, pinning, revocation freshness,
> native-loader identity, provenance, cleanup, admission, workflow, and hosted
> checks remain unchanged.

> M233 adds a test-only [Windows contained source-access source-commit Git
...
> It retrieves every bounded exact message signer `CERT_ID`, requires the
> explicit issuer/serial choice, and binds its payload to M232's same-index
> selector before and after the complete M232 boundary. Alternate identifier
> forms, signer or publisher authorization, persistent identity, pinning,
> revocation freshness, native-loader identity, provenance, cleanup,
> admission, workflow, and hosted checks remain unchanged.

> M234 adds a test-only [Windows contained source-access source-commit Git CMS
...
> It retrieves every bounded exact CMS signer record, requires version 1 and
> the explicit issuer/serial `SignerId`, and binds that copied payload to
> M233's same-state, same-index certificate ID before and after complete M233.
> Alternate CMS forms, algorithms, attributes, signature revalidation, signer
> or publisher authorization, revocation freshness, provenance, cleanup,
> admission, workflow, and hosted checks remain unchanged.

> M235 adds a test-only [Windows contained source-access source-commit Git CMS
...
> It reads the CMS `HashAlgorithm` and dedicated same-index signer hash-
> algorithm parameter, confines and copies their OID and encoded parameters,
> and requires exact equality before and after complete M234. It creates no
> algorithm policy or signature revalidation; signer authorization,
> revocation freshness, provenance, cleanup, admission, workflow, and hosted
> checks remain unchanged.

> Project status: community-alpha release candidate (`0.1.0a1`). M0 through M99 are hosted-validated and integrated into `main`; M100 through M235 are locally validated stacked milestones from the exact M99 closeout. External adoption and release-readiness observations remain explicitly bounded by the reviewed evidence records and roadmap. No public release has been made.
...
See the [architecture overview](architecture.md), [runtime contract](runtime-contract.md), [entity identity contract](ecs.md), [2D rendering contract](rendering.md), and [M4 gameplay guide](gameplay.md) before depending on these experimental APIs.
The [headless command workflow](cli-workflows.md) documents the M2 data-only
...
in-memory execution.
The [persistent command guide](commands.md) documents M119-M133 scene
...
The [agent control interface](agent-control.md) documents M5 tools, capabilities, limits, Python/CLI/MCP composition, and the Agent World Builder loop.
The [live semantic inspector guide](inspector.md) documents M10 local child
ownership, observation events, explicit write receipts, bounds, and failures.
The [rich 2D presentation guide](presentation.md) documents M11 animation,
bitmap text, tilemap, particle, and audio-mix ownership and determinism.
The [plugin compatibility guide](plugins.md) documents M12 inert manifests,
preview compatibility, deterministic reports, and the explicit no-loader boundary.
The [rollback-readiness guide](rollback-readiness.md) documents M13's
offline correction proof, external input-history limitation, network deferral,
and complete revisit gate.
The [constrained 3D decision](constrained-3d-decision.md) documents M14's
installed-surface evidence, retained layered-2D scope, and complete revisit
gate.
The [visual-editor admission decision](visual-editor-decision.md)
...
The [WASM-mod security decision](wasm-mod-security-decision.md) documents
M16's threat model, current inert boundary, prospective blockers, complete
admission gate, and explicit runtime deferral.
The [render-device conformance guide](render-device-conformance.md)
...
The [agent-tool conformance guide](agent-tool-conformance.md) documents
...
The [WorldStore conformance guide](world-store-conformance.md) documents
...
The [bounded receipt-reader guide](receipt-reader.md) documents M21's
exact v1 schema checks, limits, immutable decoding, failure behavior, and
single-version fixture non-claim under RFC-0004.
The [operation-argument compatibility guide](operation-argument-compatibility.md)
...
The [external-contributor rehearsal readiness guide](external-contributor-rehearsal-readiness.md)
...
The [external sample-game adoption readiness guide](external-sample-game-adoption-readiness.md)
...
count at zero.
The [external contributor-retention readiness guide](external-contributor-retention-readiness.md)
...
The [installation-matrix readiness guide](installation-matrix-readiness.md)
...
The [response and review latency readiness guide](response-review-latency-readiness.md)
...
The [replay-divergence-rate readiness guide](replay-divergence-rate-readiness.md)
...
The [benchmark-regression-rate readiness guide](benchmark-regression-rate-readiness.md)
...
The [agent-tool recovery-rate readiness guide](agent-tool-recovery-rate-readiness.md)
...
completion rate.
The [third-party conformance-adoption readiness guide](third-party-conformance-adoption-readiness.md)
...
The [community-alpha user guide](user-guide.md), [adapter guide](adapter-guide.md), [API policy](https://github.com/xsparc/ludoweave-engine/blob/main/API_COMPATIBILITY.md), and [release verification guide](release-process.md) cover the M6 evaluation boundary.
...
release observation. See [RFC-0101](rfcs/0101-retain-python315-prerelease-outside-support.md).

## Contributing and project policy

Contributions use the [Developer Certificate of Origin](https://github.com/xsparc/ludoweave-engine/blob/main/CONTRIBUTING.md), not a CLA. Start with the [first-contribution walkthrough](first-contribution.md) and [roadmap board](https://github.com/xsparc/ludoweave-engine/blob/main/ROADMAP.md). Please also read the [code of conduct](https://github.com/xsparc/ludoweave-engine/blob/main/CODE_OF_CONDUCT.md), [security policy](https://github.com/xsparc/ludoweave-engine/blob/main/SECURITY.md), [governance model](https://github.com/xsparc/ludoweave-engine/blob/main/GOVERNANCE.md), and repository guidance in [MAINTAINERS.md](https://github.com/xsparc/ludoweave-engine/blob/main/MAINTAINERS.md).

LudoWeave Engine is licensed under the [Apache License 2.0](https://github.com/xsparc/ludoweave-engine/blob/main/LICENSE).
