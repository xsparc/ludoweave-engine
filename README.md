...
HTTP/1.1 metadata before status or body use. `Transfer-Encoding` must be absent
or exactly `chunked` case-insensitively; it may not coexist with
`Content-Length`, whose existing bounded ASCII-decimal checks remain. RFC-0038
defines the content-silent failure boundary. This adds no alternate HTTP
client, private response-state dependency, workflow, runtime API, release
