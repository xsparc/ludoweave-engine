...
> Project status: community-alpha release candidate (`0.1.0a1`). M0 through M16 are hosted-validated and integrated into `main`. M17 is the current validation candidate: it adds an explicit installed `RenderDevice` conformance profile without discovering or admitting provider code. Existing APIs remain experimental; the M12 manifest surface is the first preview contract under RFC-0002.
